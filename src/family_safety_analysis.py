...
def analyze_aircraft_families(df):
    ...
    try:
        ...
        stats.columns = ['fatality_rate', 'total_fatalities', 'accident_count', 'first_year', 'last_year', 'hull_loss_rate']
        stats = stats.reset_index()
        
        df = df.merge(stats, on='type', how='left')
        df['service_years'] = df['last_year'] - df['first_year']
        
        # TF-IDF vectorization of aircraft types (character ngrams)
        tfidf = TfidfVectorizer(analyzer='char_wb', ngram_range=(3,5), min_df=2, lowercase=False)
        ...
        features_norm = features_norm.fillna(0)  # avoid NaNs
        
        clustering = DBSCAN(eps=0.8, min_samples=3).fit(features_norm)
...
def plot_family_analysis(df, features):
    ...
    family_stats = df.groupby('family').agg({
        'type': ['nunique', lambda x: ', '.join(pd.Series(x.unique()).astype(str).head(3)) + '...'],
    ...
    family_stats = family_stats[family_stats['family'] != -1]  # exclude noise
    ...
    fig = px.parallel_coordinates(
        ...
        dimensions=['num_types', 'total_fatalities', 'mean_fatalities', 'hull_loss_rate', 'avg_service_years', 'safety_score'],
        labels={
    ...
    safety_chart = alt.Chart(family_stats).mark_bar().encode(
        ...
        color=alt.Color('safety_score:Q', scale=alt.Scale(scheme='redyellowgreen')),
...
def aircraft_family_analysis_page(df):
    ...
    if st.button("Analyze aircraft Families", type="primary", help="An aircraft family clusters models with shared design (e.g., Boeing 737). DBSCAN groups them by names, accident rates, and service years to spot systemic risks."):
    ...
    if 'analysis_result' in st.session_state:
        ...
        if st.session_state.selected_family is not None:
            ...
            with col1:
                st.metric("Aircraft Types", family_data['type'].nunique())
            with col2:
                st.metric("Total Fatalities", int(family_data['fatalities'].sum()))
            with col3:
